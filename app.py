...
def load_data(file_path):
    if not os.path.exists(file_path): return None, None, None, None, None, None, None
    ...
    try:
        ...
        for match in match_data:
            ...
            for q in match['Quarters']:
                ...
                def add_pts(plist, w_key):
                    for p in plist:
                        player_daily_points_map.setdefault(p, {}).setdefault(dt, 0)
                        player_daily_points_map[p][dt] += w[w_key]
                
                add_pts(q['Goals'], 'Goal')
                add_pts(q['Assists'], 'Ast')
                add_pts(q['Balances'], 'Bal')
                add_pts(q['DF_CS'], 'DF')
                add_pts(q['GK_CS'], 'GK')
        ...
        return df_personal, df_merged, match_data, attendance_dict, unique_dates, player_daily_points_map, df_trend_baseline
